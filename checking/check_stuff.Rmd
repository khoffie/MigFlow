...
all_flows <- flows[, .(all_flows = sum(flows)), keyby = agegroup]
sensitive <- flows[, .(sum_flows = sum(flows)), keyby = .(agegroup, distance > 300)]
sensitive[all_flows, all := i.all_flows, on = .(agegroup)]
sensitive[, sum_flows := round(sum_flows / all, 2)]
dcast(sensitive, distance ~ agegroup, value.var = "sum_flows")
...
plot_fit(dtm[model == mod_name],
         distance, log(actual_imp / preds, base = 2), th_min = 0, p_sample = p, smooth = sm)
...
plot_fit(dtm[model %in% c(mod_name, "gravity")],
         distance, log(actual_imp / preds, base = 2), th_min = 0, p_sample = p, smooth = TRUE)
...
plot_speech <- function(dt, age, main, color = TRUE) {    
    if(color == TRUE) {
        fit <-  ggplot(dt[agegroup == age],
                       aes(distance, log(actual/preds, base = 2),
                           color = model))
    }
    if(color == FALSE) {
        fit <-  ggplot(dt[agegroup == age],
...
ggsavew(filename = "~/Documents/GermanMigration/plots/esa_both.pdf", w = 6)

plot_speech(dtm[model == "gravity2.0"], "30-50", main = "Gravity 2.0", color = FALSE)
ggsavew(filename = "~/Documents/GermanMigration/plots/esa_gravity2.0.pdf", w = 6)

plot_speech(dtm[model == "gravity1.0"], "30-50", main = "Gravity 1.0", color = FALSE)
ggsavew(filename = "~/Documents/GermanMigration/plots/esa_gravity1.0.pdf", w = 6)
<
