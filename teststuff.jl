...
p = densitychains(chain, flows, districts, 1000, 0.01)
p

fromdens, todens, densmin, densmax = densodensd(flows, districts, 1000)
p, v = densheatmap(chain, fromdens, todens, densmin, densmax)


@which densheatmap(chain, fromdens, todens, densmin, densmax, dens_th)
