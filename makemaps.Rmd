...
dt_geog[net, total := i.total, on = .(distcode = region, year, agegroup)]
...
dt_geog[net_pred, total_pred := i.total, on = .(distcode = region, year, agegroup)]
...
get_districts_with_large_net_error <- function(dt, n = 10, type = c("net", "total"), largest = TRUE) {
    type <- match.arg(type)
    if(type == "net") {
        dt[, error := (net - net_pred) / pop * 100]
        dt2 <- dt[order( - abs(error)),
                  .(distcode, agegroup, net, net_pred, error)]
    }
    if(type == "total") {
        dt[, error := (total - total_pred) / pop * 100]
        dt2 <- dt[order( - abs(error)),
                  .(distcode, agegroup, total, total_pred, error)]

    }
    if(largest == TRUE) {
        dt2 <- dt2[, head(.SD, n), keyby = agegroup]
    }
    if(largest == FALSE) {
        dt2 <- dt2[, tail(.SD, n), keyby = agegroup]
    }
    return(dt2)
}
...
errs <- get_districts_with_large_net_error(dt_geog, n = 10, type = "total", largest = TRUE)
flows_filtered <- rbindlist(lapply(1:nrow(errs), function(i) filter_rows(dt_flows, net_errs[i, ])))
flows_filtered[, type := fifelse(district == fromdist, "inflow", "outflow")]
