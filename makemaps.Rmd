...
rm_largest_flows_and_preds <- function(dt, n_rm = 8) {
    ...
    preds_from <- dt_flows[order(-preds), head(.SD, n_rm), keyby = .(fromdist, agegroup)]
    flows_to <- dt_flows[order(-flows), head(.SD, n_rm), keyby = .(todist, agegroup)]
    preds_to <- dt_flows[order(-preds), head(.SD, n_rm), keyby = .(todist, agegroup)]
    all <- rbind(flows_from, preds_from, flows_to, preds_to)[
      , .(agegroup, fromdist, todist, year)]
...
flows_constrained <- rm_largest_flows_and_preds(dt_flows)
