...
lapply(levels(ages), function(age) make_net_map(netm[agegroup == age]))

```

```{r net-migration-maps-constrained,  out.width = "140%", fig.width = 10, fig.cap = caption}

n_remove <- 16
filter1 <- dt_flows[order(-flows), head(.SD, n_remove), keyby = .(fromdist, agegroup)]
filter2 <- dt_flows[order(-preds), head(.SD, n_remove), keyby = .(fromdist, agegroup)]
filter12 <- rbind(filter1, filter2)[, .(agegroup, fromdist, todist, year)]
setkeyv(filter12, colnames(filter12))
setkeyv(dt_flows, colnames(filter12))

flows_constrained <- dt_flows[!filter12]
...
fit  <- plot_fit(dt_flows[flows > 0], (preds), log(flows / preds),
         th_min = 0, p_sample = .1, th_max = 50)
