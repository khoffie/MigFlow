...
kbl(dcast(invent, agegroup + all_actual ~ model, value.var = "all_predicted"),
    caption = "Do our models invent flows?")
...
```{r, coef-table, results = "show", eval = FALSE}
...
plot_fit(dt, distance, log(actual_imp/preds), 0, p_sample = 1, smooth = FALSE)
...
plot_fit(dt, distance, log(actual_imp / preds), th_min = 0, smooth = FALSE)
w <- 8
ggsave("~/Desktop/plot.pdf", width = w, height = w / 1.6)
